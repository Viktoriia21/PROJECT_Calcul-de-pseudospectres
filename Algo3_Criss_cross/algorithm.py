...
def get_abscissa_intersections(A, eps, x):
    ...
    # The purely imaginary eigenvalues (iy) correspond to intersections
    mask = np.abs(eigs.real) < 1e-4
    return np.sort(eigs[mask].imag)

def get_radius_intersections(A, eps, r):
    """
    Helper function for Radius: Finds the angles 'theta' where 
    the circle of radius 'r' intersects the pseudospectrum boundary.
    Uses the generalized eigenvalue problem from Equation.
    """
    n = A.shape[0]
    M = np.block([
        [-eps * np.eye(n), A],
    ...
    try:
        ...
        # Intersections occur where generalized eigenvalues have magnitude 1 (exp(i*theta))
        mask = np.abs(np.abs(eigs) - 1.0) < 1e-4
...
def criss_cross_abscissa(A, eps, tol=1e-8):
    ...
    x_vals = eigs_init[np.abs(eigs_init.real) < 1e-4].imag
...
def criss_cross_radius(A, eps, tol=1e-8):
    ...
    r_vals = eigs_rad[np.abs(eigs_rad.real) < 1e-4].imag
