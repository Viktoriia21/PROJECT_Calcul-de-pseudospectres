...
if __name__ == '__main__':
    ...
    for n in [10, 20]:
        ...
        # parallel tracing
        num_cores = 2
        print(f"starting parallel tracing using {num_cores} cores...")
        
        tasks = [(A, eps, z0, eigvals, idx, len(z_starts)) for idx, z0 in enumerate(z_starts, 1)]
        
        with multiprocessing.Pool(processes=num_cores) as pool:
            raw_contours = pool.map(trace_wrapper, tasks)
