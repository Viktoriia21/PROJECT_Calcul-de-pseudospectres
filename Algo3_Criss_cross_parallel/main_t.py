...
from test_matrices import random_matrix, linear_eigenvalue_matrix, vertical_bean_matrix
...
def find_starting_points(A, eps, eigvals, tol=1e-6):
    ...
    for i, lam in enumerate(eigvals):
        ...
        for _ in range(100):
            ...
            step = ((sigma - eps) / denom) * d0
            
            if abs(step) > 2.0:
                step = (step / abs(step)) * 2.0
...
def plot_combined_steps(A, eps, alpha, rho, eigvals, pdata_abs, pdata_rad, n):
    ...
    for contour in raw_contours:
        ...
        new_center = np.mean(contour)
        is_duplicate = False
        for existing in all_contours:
            if np.abs(np.mean(existing) - new_center) < 0.5:
                is_duplicate = True
                break
    ...
    all_x = [pt.real for c in all_contours for pt in c] + [ev.real for ev in eigvals]
    all_y = [pt.imag for c in all_contours for pt in c] + [ev.imag for ev in eigvals]
    
    if all_x and all_y:
        x_min, x_max = min(all_x) - 1.5, max(all_x) + 1.5
        y_min, y_max = min(all_y) - 2.0, max(all_y) + 2.0
        
        ax_abs.set_xlim(x_min, x_max)
        ax_abs.set_ylim(y_min, y_max)
        ax_rad.set_xlim(x_min, x_max + rho/2)
        ax_rad.set_ylim(y_min, y_max)
...
def run_benchmarks():
    ...
    A_beans = vertical_bean_matrix(10, vertical_gap=6.0, coupling=0.0)
    tests_to_run.append(("TEST with Three Vertical Components (N=30)", A_beans))
